...
from encoders import RNN_ENCODER, CNN_ENCODER
...
def stack_imgs_wo_aug(target_imgs, nn_imgs, restored_imgs, n_display, dset=None):
    wh = torch.zeros((n_display, 1, 3, 256, 256),  dtype=torch.float).to(device)
...
def stack_imgs_with_aug(target_imgs, nn_imgs, restored_imgs, n_display):
    ...
    wh = transform(torch.zeros((n_display, 1, 3, 224, 224),  dtype=torch.float)).to(device)
...
class DFLearner(pl.LightningModule):
    def __init__(self, gen, discr, discr_lambds, text_enc, cnn_enc, optim_params, nz, apply_aug):
        super().__init__()
        self.G = gen
        self.D = discr
    ...
    def on_epoch_start(self):
        ...
        for p in self.cnn_enc.parameters():
            p.requires_grad = False
        self.cnn_enc.eval()
    ...
    def forward(self, cap, cap_len, nn_imgs):
        ...
        nn_emb = self.cnn_enc(nn_imgs)

        z = torch.randn(bs, self.noise_dim).to(self.device)
        out, mu, logvar = self.G(z, cap_emb, nn_emb)
        return out, mu, logvar, cap_emb

    def training_step(self, batch, batch_idx, optimizer_idx):
        cap, cap_len, target_img, nn_imgs = batch
        gen_img, mu, logvar, cap_emb = self(cap, cap_len, nn_imgs)
        ...
        # Update G
            ...
            self.log('train_g_kl_step_loss', kl_loss),
    ...
    def validation_step(self, batch, *args):
        if self.sample_val is None:
          cap, cap_len, target_img, nn_imgs = batch
          gen_img, _, _, _ = self(cap, cap_len, nn_imgs)
...
if __name__ == '__main__':
    ...
    text_encoder = RNN_ENCODER(ntoken=train_dataset.n_words, **PARAM_MODEL['text_enc'])
    ...
    gen = Generator(**PARAM_MODEL['gen'])
    discr = Discriminator(PARAM_MODEL['discr']['ndf'])

    cnn_encoder = CNN_ENCODER(path=resnet_path)
    ...
    wandb_logger = WandbLogger(project='DF GAN', name='CA_net')
