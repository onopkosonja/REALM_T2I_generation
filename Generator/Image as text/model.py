...
class N_single_block(nn.Module):
...
class N_block(nn.Module):
    def __init__(self, nn_ch):
        super().__init__()
        self.conv_net = nn.Sequential(
            N_single_block(nn_ch, 32, stride=2, padding=1),
            N_single_block(32, 24, stride=2, padding=1),
            N_single_block(24, 16, stride=2, padding=1)
        )
    ...
    def forward(self, x):
        x = self.conv_net(x)  # 256x256x15 -> 4x4x16
        x = x.reshape(-1, 256)
        x = self.linear(x)
        return self.act(x)
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nn_ch=15):
        super().__init__()
        self.ngf = ngf
