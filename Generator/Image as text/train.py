...
def get_vars(mode, load_name=None):
    ...
    if system == 'hpc':
        ...
        if mode == 'weights':
            inception_path = os.path.join(pretrained_path, path['inception'])
    ...
    gen_path = os.path.join(pretrained_path, path['gen'])
    discr_path = os.path.join(pretrained_path, path['discr'])
    ...
    return gen_path, discr_path, rnn_path, inception_path, save_dir, load_name
...
class DFLearner(pl.LightningModule):
    ...
    def validation_epoch_end(self, *args):
        ...
        self.log('epoch', self.current_epoch)
        self.log('val imgs', [wandb.Image(grid_img)])
        self.sample_val = None
...
if __name__ == '__main__':
    ...
    gen_path, discr_path, rnn_path, _, save_dir, load_dir = get_vars('weights')
    ...
    gen = Generator(**PARAM_MODEL['gen']).to(device)
    ...
    wandb_logger = WandbLogger(project='DF GAN', name = 'train full model with imgs')
