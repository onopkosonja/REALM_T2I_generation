...
def flatten(l):
    return [item.cpu().numpy() for sublist in l for item in sublist]
...
def avg_recall(imgs_enc, caps_enc):
    ...
    res = [np.sum([x[i] for x in res], axis=0) / len(res) for i in range(len(res[0]))]
    avg_recall_caption_search, avg_recall_imgs_search = res
    return avg_recall_imgs_search, avg_recall_caption_search
