...
def calculate_activation_statistics(batches, model, classifier, out_dim):
    classifier.eval()
    model.eval()
    ...
    i = 0
    len_dataset = len(batches) * len(batches[0][0])
    act_gen = np.zeros((len_dataset, out_dim))
    act_real = np.zeros((len_dataset, out_dim))
    
    for batch in tqdm(batches, desc='activation stats loop', leave=True):
        cap, cap_len, target_img, nn_imgs = batch
        cap = cap.to(device)
        cap_len = cap_len.to(device)
        target_img = target_img.to(device)
        nn_imgs = nn_imgs.to(device)
        
        with torch.no_grad():
            gen, _ = model(cap, cap_len, nn_imgs)

        out_gen = classifier(transform(gen))[0].squeeze().cpu().numpy()
        out_real = classifier(transform(target_img))[0].squeeze().cpu().numpy()
